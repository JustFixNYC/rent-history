...
2. Add `src/api/data/client.ts` with `createDataClient()` (openapi-fetch), importing shared helpers from `src/api/shared/openapiFetch.ts` and a data-specific base URL reader in `src/api/shared/env.ts`.
