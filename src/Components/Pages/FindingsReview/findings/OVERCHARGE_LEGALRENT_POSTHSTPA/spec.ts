...
export const TYPE = "OVERCHARGE_LEGALRENT_POSTHSTPA" as const;
...
/** Keys the user may patch per row during review (OCR fields). */
export const PATCHABLE_KEYS = {
  row0: ["apt_stat", "legal_rent"] as const,
  row1: ["apt_stat", "legal_rent"] as const,
};
...
export const INTERACTIVE_STEP_IDS = ["ocr_confirm"] as const;
...
export const INTRO_VALUE_MAP = {
  ...
  rent0: (finding: Finding) =>
    finding.data.rows[ROW_INDEX.row0]?.legal_rent ?? null,
  rent1: (finding: Finding) =>
    finding.data.rows[ROW_INDEX.row1]?.legal_rent ?? null,
  percentIncrease: (finding: Finding) => {
    const rent0 = finding.data.rows[ROW_INDEX.row0]?.legal_rent;
    const rent1 = finding.data.rows[ROW_INDEX.row1]?.legal_rent;
