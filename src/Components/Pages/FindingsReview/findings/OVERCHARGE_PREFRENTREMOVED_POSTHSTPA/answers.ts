...
export type PrefrentremovedPosthstpaFormState = {
  row0AptStat: string;
  row1AptStat: string;
  ...
  row0PrefRent: string;
  row1PrefRent: string;
  row1NoPrefRent: boolean;
  getsVacancyIncrease: boolean | null;
};

export function createInitialFormState(
  finding: Finding
): PrefrentremovedPosthstpaFormState {
  ...
  return {
    ...
    row1NoPrefRent: row1?.pref_rent == null,
  ...
  return {
    rows: [
      ...
      },
      {
        ...
        pref_rent: formState.row1NoPrefRent
          ? null
          : buildRentAnswer(formState.row1PrefRent),
